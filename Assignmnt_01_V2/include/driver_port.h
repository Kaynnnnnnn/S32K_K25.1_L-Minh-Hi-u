...
#include <stdint.h>
#include "S32K144.h" /* hoặc header MCU tương ứng nếu bạn đã có */

#define DRIVER_PORT_COUNT   (5U) /* A,B,C,D,E */
...
/* Mux type — có thể mở rộng nếu cần */
typedef uint8_t Driver_PortMux;
#define DRIVER_PORT_MUX_GPIO   (1U)

/* IRQ config enum (tên giống code của bạn) */
typedef enum {
...
/* Callback type expected by your driver_port.c */
typedef void (*Driver_PortCallback)(Driver_PortInstance port, uint8_t pin);

/* Prototypes — bạn cần implement trong driver_port.c */
